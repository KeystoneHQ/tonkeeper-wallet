...
import { KeystoneScanQRProps, KeystoneScanState } from './KeystoneScanQR.interface';
import * as S from '../ScanQR/ScanQR.style';
...
export const KeystoneScanQR: FC<KeystoneScanQRProps> = ({ route }) => {
  ...
  const [decoder, setDecoder] = useState(new URDecoder());
  ...
  const onRead = async (e: any) => {
    if (e.data) {
      decoder.receivePart(e.data);
      if (decoder.isSuccess()) {
        const ur = decoder.resultUR();
        const status = await onScan(ur);
